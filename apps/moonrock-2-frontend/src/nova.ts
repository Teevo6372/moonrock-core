...
app.innerHTML = `
  <main class="shell nova-page-shell">
    <section class="hero nova-page-hero">
      ...
      <div class="hero-cta">
        <p id="status" class="status" aria-live="polite">Nova is getting things ready…</p>
      </div>
    </section>
...
function friendlyPrompt(question: DiscoveryQuestion): string {
  const prompts: Record<string, string> = {
    ...
    businessChallenges: "Before we get into numbers, what feels harder than it should in the business right now?",
    ...
    departmentsAffected: "Looking at what we've uncovered, which parts of the business seem tied into the same problem?",
...
async function begin(): Promise<void> {
  ...
  try {
    const response = await startDiscovery(sessionId, currentPath);
    setBusy(false);
    renderResponse(response, true);
    visualStage.playTransientState("speaking");
    window.setTimeout(() => visualStage.playBehavior("energetic"), 1600);
    panel.scrollIntoView({ behavior: "smooth", block: "start" });
    status.textContent = "Nova is with you.";
  } catch (error) {
    setBusy(false);
