...
export function createDiscoveryRouter(repository: DiscoveryStateRepository = new InMemoryDiscoveryStateRepository(), options: DiscoveryRouterOptions = {}): Hono {
  const router = new Hono(); const conversationEngine = options.conversationEngine ?? new SessionGroundedNovaConversationEngine();

  router.post("/:sessionId/start", async (context) => {
    const sessionId = context.req.param("sessionId"); const body = await context.req.json() as { path?: unknown; visitorId?: unknown; conversationId?: unknown; previousConversationSummary?: unknown };
    if (body.path !== "startup" && body.path !== "existing_business") return context.json({ code: "INVALID_DISCOVERY_PATH" }, 400);
    const visitorId = typeof body.visitorId === "string" && body.visitorId.trim() ? body.visitorId.trim() : `anonymous-${sessionId}`; const conversationId = typeof body.conversationId === "string" && body.conversationId.trim() ? body.conversationId.trim() : sessionId; const previousConversationSummary = typeof body.previousConversationSummary === "string" && body.previousConversationSummary.trim() ? body.previousConversationSummary.trim() : undefined;
    ...
    const openingText = body.path === "startup" ? "I'm starting something." : "My business needs to grow."; const conversationTurn = await conversationEngine.respond(result.state, openingText, { opening: true, progressPercent: 0 });
  ...
  router.post("/:sessionId/answers", async (context) => {
    const sessionId = context.req.param("sessionId"); const current = await repository.load(sessionId); if (!current) return context.json({ code: "DISCOVERY_NOT_FOUND" }, 404); if (current.state.completed) return context.json({ code: "DISCOVERY_COMPLETE" }, 409);
    const body = await context.req.json() as { field?: unknown; value?: unknown; identity?: ProductionGhlContactIdentity }; if (typeof body.field !== "string" || !("value" in body)) return context.json({ code: "INVALID_DISCOVERY_ANSWER" }, 400);
    const rawCustomerText = typeof body.value === "string" ? body.value : String(body.value);
    if (isHumanHandoffRequest(rawCustomerText)) { const conversationTurn = await conversationEngine.respond(current.state, rawCustomerText); return context.json({ path: current.state.path, completed: current.state.completed, progress: { answered: Object.keys(current.state.answers).filter((key) => key !== "path").length, requiredRemaining: 0 }, progressiveFlightPlan: { phase: "listening", summary: "Discovery paused for a human handoff.", signals: [] }, conversationTurn, humanHandoff: { status: "contact_required", requestText: rawCustomerText, message: "Nova paused discovery. Add your contact details and Moonrock can continue from what you've already shared." }, view: { ...toImmersiveNovaView({ path: current.state.path, completed: false, progress: { answered: 0, requiredRemaining: 0 }, progressiveFlightPlan: { phase: "listening", summary: "Discovery paused for a human handoff.", signals: [] } }), visualState: "handoff" } }); }
    ...
    const view = toImmersiveNovaView(result.response); const journey = result.response.completed && result.response.result ? completedJourney(result.response.result.flightPlan) : journeyForProgress(view.progressPercent, false);
    ...
    const conversationTurn = result.response.clarification ? { answer: result.response.clarification.message, mode: "grounded_fallback" as const, intent: "pause_discovery" as const } : result.response.completed ? { answer: "I have enough to give you a useful starting direction. Here's your Preliminary Flight Plan; anything still unknown is an assumption we can fine-tune after you see the recommendation.", mode: "grounded_fallback" as const, intent: "answer_question" as const } : await conversationEngine.respond(result.state, rawCustomerText, { progressPercent: view.progressPercent, ...(result.response.nextQuestion ? { nextNeed: { field: String(result.response.nextQuestion.field), prompt: result.response.nextQuestion.prompt } } : {}) });
  ...
  router.post("/:sessionId/conversation", async (context) => {
    const sessionId = context.req.param("sessionId"); const current = await repository.load(sessionId); if (!current) return context.json({ code: "DISCOVERY_NOT_FOUND" }, 404); const body = await context.req.json() as { question?: unknown }; if (typeof body.question !== "string" || !body.question.trim()) return context.json({ code: "INVALID_NOVA_QUESTION" }, 400);
    if (!current.state.completed && isFlightPlanRequest(body.question)) {
      const result = requestPreliminaryFlightPlan(current.state); try { await repository.save(sessionId, result.state, current.version); } catch { return context.json({ code: "DISCOVERY_VERSION_CONFLICT" }, 409); }
      const view = toImmersiveNovaView(result.response); return context.json({ ...result.response, conversationTurn: { answer: "Absolutely. I'm stopping discovery here and showing you the best Preliminary Flight Plan I can build from what you've already told me.", mode: "grounded_fallback", intent: "answer_question" }, journey: result.response.result ? completedJourney(result.response.result.flightPlan) : journeyForProgress(100, true), view });
    }
